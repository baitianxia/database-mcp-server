...
- 安装完成后只生成了 `%USERPROFILE%\database-mcp-server\claude-code.mcp.json`，Claude Code 的 `/mcp` 中没有 `database-mcp`。
- 哈希和 MCP 冒烟通过后，`Move-Item` 将 `.staging-<PID>-<timestamp>` 发布为版本目录时报告“访问被拒绝”，安装回滚并退出。
- 候选包清单/目录版本与 MCP `serverInfo.version` 曾不一致：包目录为新版本，入口源码仍硬编码旧版本号。
...
5. 安装器把旁路注册 JSON 当成了 Claude Code 注册结果，但没有调用 Claude Code CLI 的 `mcp add --scope user`，也没有读取用户级配置做安装后核对；因此安装成功和 Claude Code 实际可发现之间没有闭环。
6. 发布暂存目录只使用一次 `Move-Item`，没有针对 Windows 安全软件或原生进程短暂占用的同卷目录重命名重试；同时服务版本号没有和 `package.json`/发布清单共用来源。
...
- `INSTALL.ps1` 复用已有 `claude.exe`/`claude.cmd`，执行 `mcp remove/add/get --scope user` 注册 `database-mcp`，核对用户配置中的 Node、入口和 `DATABASE_CONFIG_PATH`；注册失败恢复原 Claude 用户配置并使安装失败。
- `UNINSTALL.ps1` 在删除本地版本前注销同一个用户级 MCP 条目，避免留下指向已删除版本的 Claude 配置。
- 版本目录发布改用同卷 `[IO.Directory]::Move`，在源/目标状态明确且确认是可重试的权限或 IO 错误时指数退避重试；持续失败会保留原始错误并不覆盖目标目录。服务版本号同步更新，冒烟响应应与发布版本一致。
...
- 当前候选包：`database-mcp-server-0.2.7-windows-x64.zip`；SHA-256 为 `352832454811e025592d2a509d1b6dfe9af745173e805652968480f3ffc13a0c`，来源提交为 `f93a323`，状态仍为 `CANDIDATE_UNVERIFIED`。
